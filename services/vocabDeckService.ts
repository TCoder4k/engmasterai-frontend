...
export const getPublishedDeck = async (id: string): Promise<VocabDeck> => {
  const response = await apiFetch(`${API_BASE_URL}/vocab/decks/${id}`);

  if (!response.ok) return throwApiError(response, 'Failed to load vocabulary deck');
...
export const getPublishedDeckWords = async (deckId: string): Promise<VocabWordListResponse> => {
  const response = await apiFetch(`${API_BASE_URL}/vocab/decks/${deckId}/words`);
...
export const getPublishedDecksByLibrary = async (libraryId: string): Promise<VocabDeckListResponse> => {
  const response = await apiFetch(`${API_BASE_URL}/vocab/libraries/${libraryId}/decks`);
...
export const getManagedDecksByLibrary = async (libraryId: string): Promise<ManagedVocabDeckListResponse> => {
  const response = await apiFetch(`${API_BASE_URL}/vocab/libraries/${libraryId}/decks/manage`);
...
export const publishDeck = async (id: string): Promise<ManagedVocabDeck> => {
  const response = await apiFetch(`${API_BASE_URL}/vocab/decks/${id}/publish`, {
    method: 'PATCH',
  });
...
export const unpublishDeck = async (id: string): Promise<ManagedVocabDeck> => {
  const response = await apiFetch(`${API_BASE_URL}/vocab/decks/${id}/unpublish`, {
    method: 'PATCH',
  });
...
export const getManagedDeckWords = async (deckId: string): Promise<ManagedVocabDeckWordListResponse> => {
  const response = await apiFetch(`${API_BASE_URL}/vocab/decks/${deckId}/words/manage`);
...
export const detachWord = async (deckId: string, wordId: string): Promise<void> => {
  const response = await apiFetch(`${API_BASE_URL}/vocab/decks/${deckId}/words/${wordId}`, {
    method: 'DELETE',
  });
